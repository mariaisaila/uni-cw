...
const websiteURL = 'https://mariaisaila.github.io/uni-cw/';
...
// add a vehicle (owner already there)
test('add a vehicle with owner', async ({page}) => {
   await page.getByRole('link', { name: 'Add a vehicle' }).click();
   await page.locator('#rego').fill('NW51STY')
   await page.locator('#make').fill('Porsche')
   await page.locator('#model').fill('Taycan')
   await page.locator('#colour').fill('white')
   ...
   await page.locator('#rego').fill('NW51STY')
   await page.getByRole('button', { name: 'Submit' }).click();
   await expect(page.locator('#results')).toContainText('Porsche')
   await expect(page.locator('#results').locator('div')).toHaveCount(1)
})

// add a vehicle (vehicle information missing)
...
// add a vehicle (person information missing)
test('add a vehicle with owner missing information', async ({page}) => {
   await page.getByRole('link', { name: 'Add a vehicle' }).click();
   await page.locator('#rego').fill('NW51STY')
   await page.locator('#make').fill('Porsche')
   await page.locator('#model').fill('Taycan')
   await page.locator('#colour').fill('white')
   await page.locator('#owner').fill('Maria')
   await page.getByRole('button', { name: 'Add vehicle' }).click();
...
// add a vehicle (add an owner with an id that already exists)
test('add a vehicle owner id taken', async ({page}) => {
   await page.getByRole('link', { name: 'Add a vehicle' }).click();
   await page.locator('#rego').fill('LKJ23UO')
   await page.locator('#make').fill('Porsche')
   await page.locator('#model').fill('Taycan')
   ...
   await expect(page.locator('#message')).toContainText('Error adding vehicle, please check console for details.')

})

// add a vehicle (with a plate number that already exists)
test('add a vehicle id taken', async ({page}) => {
   await page.getByRole('link', { name: 'Add a vehicle' }).click();
   await page.locator('#rego').fill('LKJ23UO')
   await page.locator('#make').fill('Porsche')
   await page.locator('#model').fill('Taycan')
